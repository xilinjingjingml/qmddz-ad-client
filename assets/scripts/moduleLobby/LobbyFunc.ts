...
import { checkPhoneBinding, czcEvent, iMessageBox, playAD, showAwardResultPop, socialShare, isFreeAdvert, checkTimeLimitBox, TimeFormat } from "../base/BaseFuncTs";
...
export function loadLotteryData(callback: () => void = null, channel: number = 2) {
    const datakey = channel == 2 ? "lotteryData" : "happyLotteryData"//todot channel == 1 ? "luckyLotteryData" : 
...
export function loadAdConfig(callback: () => void = null) {
    ...
    http.open(DataManager.getURL("AD_CONFIG"), param, (res) => {
        if (res && res.adConfig) {
            ...
            console.log("jin---DataManager.CommonData.AdConfig", config)
            callback && callback()
...
export function isShowPayPage(){
    ...
    if(cc.sys.os == cc.sys.OS_IOS){
        ...
        return DataManager.Instance.onlineParam.round_pay_ios <= DataManager.CommonData["roleCfg"]["roundSum"] && DataManager.Instance.onlineParam.isShowShop_ios === 1//TODOT
    }else{
