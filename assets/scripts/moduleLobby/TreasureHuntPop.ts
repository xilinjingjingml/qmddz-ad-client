...
import { checkAdCanReceive, getAdLeftTimes, getNextAdType, receiveAdAward } from "./LobbyFunc"
...
export default class TreasureHuntPop extends BaseScene {
    ...
    updateState() {
        const type = getNextAdType(AdsConfig.taskAdsMap.TreasureHunt)
        ...
        for (const chest of chests) {
            const btnAd = chest.getChildByName("btnGetAward")
            btnAd.getComponent(cc.Button).interactable = canReceive
            btnAd.active = type == 2

            const btnShare = chest.getChildByName("btnGetAwardShare")
            btnShare.getComponent(cc.Button).interactable = canReceive
            btnShare.active = type != 2
        }
