...
export default class MatchAwardHistory extends BaseComponent {
    ...
    http_getAward(matchInfo) {
        ...
        BaseFunc.HTTPGetRequest(DataManager.getURL("MATCH_GET_AWARD"), params, (res) => {
            cc.log(res)
            if (res) {
                if (res.ret == 0) {
                    if (!!res.reward) {
    ...
    http_getAccumulateData() {
        ...
        BaseFunc.HTTPGetRequest(DataManager.getURL("GET_MATCH_AWARD_COUNT"), params, (res) => {
            cc.log(res)
            if (res) {
                if (res.ret == 0) {
    ...
    http_getHistoryData(page, size, htype, callback) {
        ...
        BaseFunc.HTTPGetRequest(DataManager.getURL("MATCH_HISTORY"), params, (res) => {
            cc.log(res)
            if (res) {
                if (res.ret == 0) {
                    if (!!callback) {
