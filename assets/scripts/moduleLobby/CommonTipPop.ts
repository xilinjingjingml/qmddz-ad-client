...
export default class CommonTipPop extends BaseScene {
    ...
    onOpenScene() {
        ...
        //TODO 1.隐私协议 2.用户协议 3.转盘规则 4.合成转盘 5.双旦嘉年华（奖励预览）
        if(this.initParam.idx == 0){
            const title = cc.find("nodeMain/titlebg/title_" + this.initParam.idx,this.node)
            title.active = true
            for(let i = 1; i < 11; i++){
                cc.loader.load({url: "https://pictures.hiigame.com/qmddz/lb_" + i + ".png", type: 'png'}, (err, tex)=>{
                    if (err) {
            ...
            title.active = true
            for(let i = 1; i < 20; i++){
                cc.loader.load({url: "https://download.mcbeam.cc/Image/yhxy_" + i + ".png", type: 'png'}, (err, tex)=>{
                    if (err) {
                        cc.error(err.message || err)
                        return
                    }
        ...
        else if(this.initParam.idx == 4){
            const title = cc.find("nodeMain/titlebg/title_" + 2,this.node)
            title.active = true
            const content_doubleEgg = cc.find("nodeMain/NewScrollView/view/content/rule_doubleEgg", this.node)
            content_doubleEgg.active = true
        }
        else if(this.initParam.idx == 5){
            const title = cc.find("nodeMain/titlebg/title_" + this.initParam.idx, this.node)
            title.active = true
