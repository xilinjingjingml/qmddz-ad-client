...
export default class Chat extends BaseComponent {
    ...
    initViewText() {
        ...
        chatTexts.forEach((data, i) => {
            ...
            functions.find($, "line").active = i != 0
            functions.find($, "label", cc.Label).string = data.text
    ...
    initViewEmoji() {
        ...
        chatEmojis.forEach((data, i) => {
            ...
            NodeExtends.setNodeSprite({
                node: functions.find($, "icon"),
                url: "moduleRPDdzRes/images/Chat/" + data.icon,
                delayShow: true
            })
    ...
    sendChat(type: ChatType, index: string) {
        GameLogic.Instance().sendMessage<Iproto_cb_chat_req>({
            opcode: 'proto_cb_chat_req',
            type: 0,
            message: ChatPrefix[type] + index
        })
        this.$("node_chat").active = false
    }
