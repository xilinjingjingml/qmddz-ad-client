...
export default class MatchGame extends BaseComponent {
    ...
    messageRank: proto.Iproto_bc_match_rank_not
    messageInfo: proto.Iproto_bc_match_info_noti
    messageRelive: proto.Iproto_bc_match_relive_noti
    ...
    proto_bc_match_scores_not(event) {
        const message: proto.Iproto_bc_match_scores_not = event.packet
    ...
    proto_bc_match_info_noti(event) {
        const message: proto.Iproto_bc_match_info_noti = event.packet
        this.messageInfo = message
    }

    proto_bc_match_rank_not(event) {
        const message: proto.Iproto_bc_match_rank_not = event.packet
    ...
    proto_bc_match_weed_out_score_not(event) {
        const message: proto.Iproto_bc_match_weed_out_score_not = event.packet
    ...
    proto_bc_match_relive_noti(event) {
        const message: proto.Iproto_bc_match_relive_noti = event.packet
    ...
    proto_bc_match_relive_ack(event) {
        const message: proto.Iproto_bc_match_relive_ack = event.packet
        if (message.ret == 0) {
            // iMessageBox("复活成功")
