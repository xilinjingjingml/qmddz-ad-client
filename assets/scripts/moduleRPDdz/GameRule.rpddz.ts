...
namespace GameRule {
	...
	let m_cards: ICard[] = [] //处理的牌
	let m_typeNum: number = -1

	export let m_tipCards: ICard[][] = [] //提示牌
	export const m_chooseCardType: ICardsType = { mNTypeValue: 0, mNTypeNum: 0, mNTypeBomb: 0 }
	...
	export function checkChooseCardsType(cards: ICard[], handCards: ICard[], cardType: ICardsType) {
		m_cards = cards
	...
	// 检测牌的完整性 橙红的话 设置选中牌
	function checkCardsIntact(handCards: ICard[]) {
		let cardCount = 0
	...
	// 比较大小
	function compareCardsType(cardType: ICardsType) {
		// 不出
	...
	export function checkCardsType(cards: ICard[]) {
		m_cards = cards
		m_typeNum = 0
	...
	// 提示出牌
	export function tips(cards: ICard[], cardType: ICardsType) {
		m_cards = cards
		m_tipCards = []
	...
	function addTipsGroup(num: number, value: number, tipCards?: ICard[][]) {
	...
	function addTipsCards(cards: ICard[], num: number, value: number) {
		for (let i = 0; i < num; i++) {
			cards.push({
				mNValue: value,
				mNColor: -1,
	...
	function tipsSearch1(cardType: ICardsType) {
	...
	function tipsSearch2(cardType: ICardsType) {
		tipsSearchSame(cardType, 2)
	}

	function tipsSearch3(cardType: ICardsType) {
		tipsSearchSame(cardType, 3)
	}

	function tipsSearchSame(cardType: ICardsType, same: number) {
		const tempTipsCards = []
	...
	function tipsSearch411(cardType: ICardsType) {
	...
	function tipsSearch422(cardType: ICardsType) {
		for (let i = cardType.mNTypeValue + 1; i < ECardPoint.Wang; i++) {
			if (m_cardPointCount[i] < 4) {
				continue
			}
	...
	function tipsSearchCardInfo(cardInfos: ICardInfos, func: (cardType: ICardsType, long: number) => void, cardType: ICardsType) {
	...
	function tipsSearch1s(cardType: ICardsType, long: number) {
		tipsSearchSeries(cardType, 1, long)
	}

	function tipsSearch2s(cardType: ICardsType, long: number) {
		tipsSearchSeries(cardType, 2, long)
	}

	function tipsSearch3s(cardType: ICardsType, long: number) {
		tipsSearchSeries(cardType, 3, long)
	}

	function tipsSearchSeries(cardType: ICardsType, same: number, long: number) {
	...
	function tipsSearch31s(cardType: ICardsType, long: number) {
	...
	function tipsSearch32s(cardType: ICardsType, long: number) {
		let maxValue = long == 1 ? ECardPoint.Wang : ECardPoint.Wang - long
		for (let i = cardType.mNTypeValue + 1; i < maxValue; i++) {
			let find = true
			for (let j = 0; j < long; ++j) {
	...
	function tipsSearchBomb(cardType: ICardsType) {
		//硬炸弹
