...
import { copyToClipBoard, czcEvent, extendMatchLogic, getNewBieRoundLimit, getRedPacketAwardConfig, iMessageBox, playADBanner, share, 
	showAwardResultPop, numberFormat, uploadKuaiShou, MsgBox, zeroDate, getShopBox } from "../base/BaseFuncTs";
import SceneManager from "../base/baseScene/SceneManager";
import GameMagicEmoji from "../moduleBase/GameMagicEmoji";
import { check03ExchangeGoods, checkAdCanReceive, checkTomorrowStatus, getAllAdCountTimes, isShowPayPage, receiveAdAward, getReliefState } from "../moduleLobby/LobbyFunc";
...
export default class GameScene extends GameSceneStateController {
	...
	proto_gc_game_start_not_handler(event) {
		...
		if(DataManager.CommonData["roleCfg"]["roundSum"] < 4){
			czcEvent("游戏-牌局开始-" + DataManager.CommonData["roleCfg"]["roundSum"] + "局" )
		}
	...
	onChuPai() {
		//1.
		if(DataManager.CommonData["roleCfg"]["roundSum"] < 4){
			czcEvent("游戏-牌局中-玩家手动出牌-" + DataManager.CommonData["roleCfg"]["roundSum"] + "局")
		}
	...
	onPressTuoGuan(EventTouch, data) {
		...
		AudioManager.playButtonSound()
		if(DataManager.CommonData["roleCfg"]["roundSum"] < 4){
			czcEvent("游戏-牌局中-取消托管-" + DataManager.CommonData["roleCfg"]["roundSum"] + "局")
		}
	...
	onPressExit(EventTouch, data) {
		...
		if(DataManager.CommonData["roleCfg"]["roundSum"] < 4){
			czcEvent("游戏-牌局界面-返回大厅-" + DataManager.CommonData["roleCfg"]["roundSum"] + "局")
		}
		cc.log("onPressExit")
